...
export const getProduct =
  ...
  async (dispatch) => {
    try {
      ...
      let link = `/products/product?keyword=${keyword}&page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&ratings[gte]=${ratings}`;

      if (category) {
        link = `/products/product?keyword=${keyword}&page=${currentPage}&price[gte]=${price[0]}&price[lte]=${price[1]}&category=${category}&ratings[gte]=${ratings}`;
      }
...
export const deleteReviews = (reviewId, productId) => async (dispatch) => {
  try {
    ...
    const { data } = await axios.delete(
      `/products/reviews?id=${reviewId}&productId=${productId}`
    );
